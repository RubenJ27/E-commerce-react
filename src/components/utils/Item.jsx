...
const Item = ({ifExists, id, color, shadow, title, text, img, btn, rating, price }) => {
  return (
    <>
    <div className={`w-full relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ ifExists ? 'justify-items-start' : 'justify-items-center'} rounded-xl py-4 px-5 transition-all duration-700 ease-in-out hover:scale-105`}>
      <div className={`grid items-center ${
        ...
        <div className='flex items-center justify-between w-28 my-2'>
          <div className='flex items-center blur-effect-theme bg-white/80 px-1 rounded'><h1 className='text-black text-sm font-medium'>${price}</h1></div>
